...
class ActiveLearner():
    ...
    def custom_query_strategy(self, model, sys_list, X_index_list, n_instances, used_data, used_label, metric, K, Q, calculator, seed_initial):
        ...
        for sys_idx, sys in enumerate(sys_list):
            ...
            if each_system.shape[0] >= n_instances:
                ...
                if np.unique(each_system, axis=0).shape[0] >= n_instances:
                    ...
                    top_query_index = self.multi_argmin(utility, n_instances=n_instances)
        ...
        max_sys_idx = self.multi_argmin(np.asarray(candidate_uncertainty_values), self.B)
    ...
    def K_means_Plus_Plus(self, seleceted_system, n_instances, top_all_utility, seed_initial):
        # K means++ 
        centers, center_id = kmeans_plusplus(seleceted_system, n_clusters=n_instances, 
                                           random_state=seed_initial, n_local_trials=None)
        ...
        for i in range(n_instances):
            ...
            if len(cluster_points) > 0:
                # get the sample with the smallest score
                cluster_utilities = top_all_utility[cluster_points]
                best_point_idx = cluster_points[np.argmin(cluster_utilities)]
        ...
        if empty_clusters:
            ...
            # remaining samples that have been selected
